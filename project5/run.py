...
def bold(s):
    print
    bcolors.BOLD + bcolors.UNDERLINE + s + bcolors.ENDC
...
class Config:
    def __init__(self, filename):
        ...
        self.app_batch_frac = self.__get_default__(conf['tests'], 'maximum_appends_batched_fraction', 0.0, 1.0,
                                                   APPENDS_BATCHED_FRAC,
                                                   'Fraction of appends that may be batched must be between 0 and 1 (default: %f)' % (
                                                       APPENDS_BATCHED_FRAC))
        ...
        for test, tiers in conf['tests']['benchmarks'].iteritems():
            if len(tiers) != 3:
                raise ValueError(
                    "Incorrect number of performance tiers in test %s. len(tiers) = %i" % (test, len(tiers)))
    ...
    def dump(self):
        ...
        for event in self.events:
            print
            '%8s %15s %s' % ('Event', event['type'], event['time'])

        for test, tiers in self.benchmarks:
            print
            '%16s %s' % (test, str(tiers))
...
class Stats:
    ...
    def dump(self):
        ...
        print
        'Replicas that died/were killed: %i/%i' % (self.died, self.killed)
        print
        'Total messages sent:', self.total_msgs
        print
        'Total messages dropped:', self.total_drops
        print
        'Total client get()/put() requests: %i/%i' % (self.total_get, self.total_put)
        print
        'Total duplicate responses:', self.duplicates
        print
        'Total unanswered get()/put() requests: %i/%i' % (self.unanswered_get, self.unanswered_put)
        print
        'Total redirects:', self.redirects
        print
        'Total get()/put() failures: %i/%i' % (self.failed_get, self.failed_put)
        print
        'Total get() with incorrect response:', self.incorrect
        if len(self.latencies) > 0:
            print
            'Mean/Median query latency: %fsec/%fsec' % (float(sum(self.latencies)) / len(self.latencies),
...
class Client:
    ...
    def finalize(self):
        for req in self.reqs.itervalues():
            if req.get:
                self.sim.stats.unanswered_get += 1
            else:
    ...
    def deliver(self, raw_msg):
        if self.alive:
            try:
                ...
                print
                '*** Simulator Error - Unable to send to replica'
                self.shutdown()
        return False
...
class Simulation:
    ...
    def run(self):
        ...
        # give the OS scheduler a moment to run other proceses, including the replicas
        time.sleep(0.1)

        # the main event loop
        while clock - start < self.conf.lifetime and self.stats.total_msgs < self.conf.max_packets and len(
                self.living_rids) > 0:
        ...
        if self.stats.total_msgs >= self.conf.max_packets:
            print
            "*** Simulator Error - Replicas sent too many packets (>%i), possible packet storm" % (
                self.conf.max_packets)
    ...
    def __kill_non_leader__(self):
        if len(self.living_rids) > 1:
            ...
            print
            '*** Simulator Error - too few living replicas to kill another (%i)' % (len(self.living_rids))
    ...
    def correctness_check(self, verbose=True):
        ...
        if self.stats.unanswered_get > self.stats.generated_get * self.conf.max_get_frac:
            ...
            if verbose: fail('\tError: insufficient get() requests answered (%i > %i * %.2f)' % (
            self.stats.unanswered_get, self.stats.generated_get, self.conf.max_get_frac))
            passed = False
        if self.stats.unanswered_put > self.stats.generated_put * self.conf.max_put_frac:
            # Your system must answer a minimal number of get requests from clients
            if verbose: fail('\tError: insufficient put() requests answered (%i > %i * %.2f)' % (
            self.stats.unanswered_put, self.stats.generated_put, self.conf.max_put_frac))
            passed = False
        if self.stats.failed_to_generate_get > self.stats.generated_get * self.conf.max_get_fail_frac:
            ...
            if verbose: fail(
                '\tError: insufficient get() requests were generated because insufficient put()s were accepted (%i > %i * %.2f)' % (
                self.stats.failed_to_generate_get, self.stats.generated_get, self.conf.max_get_fail_frac))
            passed = False
        if self.stats.total_msgs < self.conf.requests * self.conf.replicas * (1 - self.conf.mix) * (
                1 - self.conf.app_batch_frac):
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print
        'Usage: $ %s <config file>' % (sys.argv[0])
        sys.exit()
